...
const BASE_ARMS_DOWN = {
  ...
  leftHand:      { euler: [0.1, 0, 0] },
  rightHand:     { euler: [0.1, 0, 0] },
  
  leftIndexProximal:      { euler: [0.25, 0, 0] },
  leftIndexIntermediate:  { euler: [0.20, 0, 0] },
  leftIndexDistal:        { euler: [0.15, 0, 0] },
  leftMiddleProximal:     { euler: [0.30, 0, 0] },
  leftMiddleIntermediate: { euler: [0.25, 0, 0] },
  leftMiddleDistal:       { euler: [0.15, 0, 0] },
  leftRingProximal:       { euler: [0.35, 0, 0] },
  leftRingIntermediate:   { euler: [0.25, 0, 0] },
  leftRingDistal:         { euler: [0.15, 0, 0] },
  leftLittleProximal:     { euler: [0.40, 0, 0] },
  leftLittleIntermediate: { euler: [0.25, 0, 0] },
  leftLittleDistal:       { euler: [0.15, 0, 0] },
  leftThumbProximal:      { euler: [0, 0, 0.2] },
  leftThumbDistal:        { euler: [0, 0, 0.1] },

  rightIndexProximal:      { euler: [0.25, 0, 0] },
  rightIndexIntermediate:  { euler: [0.20, 0, 0] },
  rightIndexDistal:        { euler: [0.15, 0, 0] },
  rightMiddleProximal:     { euler: [0.30, 0, 0] },
  rightMiddleIntermediate: { euler: [0.25, 0, 0] },
  rightMiddleDistal:       { euler: [0.15, 0, 0] },
  rightRingProximal:       { euler: [0.35, 0, 0] },
  rightRingIntermediate:   { euler: [0.25, 0, 0] },
  rightRingDistal:         { euler: [0.15, 0, 0] },
  rightLittleProximal:     { euler: [0.40, 0, 0] },
  rightLittleIntermediate: { euler: [0.25, 0, 0] },
  rightLittleDistal:       { euler: [0.15, 0, 0] },
  rightThumbProximal:      { euler: [0, 0, -0.2] },
  rightThumbDistal:        { euler: [0, 0, -0.1] },
...
class GestureEngine {
  ...
  update(t, delta) {
    ...
    for (const [boneName, transform] of Object.entries(BASE_ARMS_DOWN)) {
      ...
      const q = new Q();
      const isHandOrFinger = /(Hand|Thumb|Index|Middle|Ring|Little)/i.test(boneName);
      
      if (transform.euler) {
        if (!this.isVrm && isHandOrFinger) {
          let [ex, ey, ez] = transform.euler;
          const isLeft = boneName.toLowerCase().includes('left');
          q.setFromEuler(new E(ez, ey, isLeft ? -ex : ex));
        } else {
          q.setFromEuler(new E(...transform.euler));
        }
      } else if (transform.axis && transform.angle !== undefined) {
        if (!this.isVrm && isHandOrFinger) {
          let [ax, ay, az] = transform.axis;
          const isLeft = boneName.toLowerCase().includes('left');
          q.setFromAxisAngle(new this.THREE.Vector3(az, ay, isLeft ? -ax : ax).normalize(), transform.angle);
        } else {
    ...
    if (gesture.oscillation) {
      for (const [boneName, osc] of Object.entries(gesture.oscillation)) {
        ...
        if (!this.isVrm && isHandOrFinger) {
          ...
          const newX = az;
          const newY = ay;
          const newZ = isLeft ? -ax : ax;
          ax = newX; ay = newY; az = newZ;
        }

        const qOsc = new Q().setFromAxisAngle(new this.THREE.Vector3(ax, ay, az).normalize(), offset);
